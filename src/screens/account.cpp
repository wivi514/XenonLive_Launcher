...
void Problem(const std::string& code) {
    ...
    if (code == "bad_email") help = "That does not look like an email address.";
    else if (code == "taken") help = "Another account already uses that address.";
    else if (code == "bad_country") help = "Two letters: CA, US, FR, JP...";
    else if (code == "bad_credentials") help = "The current password is wrong.";
    else if (code == "bad_password") help = "A password is at least 8 characters.";
    else if (code == "mismatch") help = "The two new passwords differ.";
    else if (code == "signed_out" || code == "unreachable") help = "The server could not be reached.";
...
void DrawAccount(App& app) {
    ...
    // -- recovery email ----------------------------------------------------
    xlive::theme::Section("Recovery email");
    if (!st.email_filled) {
        std::snprintf(st.email, sizeof(st.email), "%s", id.email.c_str());
        st.email_filled = true;
    }
    if (id.email.empty()) {
        ImGui::TextColored(xlive::theme::kAmber, "None. If you forget your password, this account cannot be recovered.");
    } else {
        ImGui::Text("A code can be sent to %s if you forget your password.", id.email.c_str());
    }
    ImGui::PushStyleColor(ImGuiCol_Text, kMuted);
    ImGui::TextWrapped("Only for getting back in. Nothing else is ever sent to it - no news, no "
                       "updates - and it is never shown to other players.");
    ImGui::PopStyleColor();
    {
        const bool busy = st.email_ticket != 0;
        ImGui::BeginDisabled(busy);
        ImGui::SetNextItemWidth(box);
        const bool enter = ImGui::InputTextWithHint("##email", "you@example.org", st.email,
                                                    sizeof(st.email),
                                                    ImGuiInputTextFlags_EnterReturnsTrue);
        ImGui::SameLine();
        const bool changed = std::string(st.email) != id.email;
        ImGui::BeginDisabled(!changed || st.email[0] == '\0');
        if (ImGui::Button("Save##email") || (enter && changed && st.email[0])) app.SaveEmail(false);
        ImGui::EndDisabled();
        if (!id.email.empty()) {
            ImGui::SameLine();
            ImGui::PushStyleColor(ImGuiCol_Button, kPanelHi);
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered, kRed);
            if (ImGui::Button("Remove##email")) app.SaveEmail(true);
            ImGui::PopStyleColor(2);
        }
        ImGui::EndDisabled();
        if (busy) ImGui::TextDisabled("Saving...");
    ...
    // -- country -----------------------------------------------------------
    xlive::theme::Section("Country");
    if (!st.country_filled) {
        std::snprintf(st.country, sizeof(st.country), "%s", id.country.c_str());
        st.country_filled = true;
    }
    ImGui::PushStyleColor(ImGuiCol_Text, kMuted);
    ImGui::TextWrapped("Two letters, shown on your gamercard to friends. Optional.");
    ImGui::PopStyleColor();
    {
        const bool busy = st.country_ticket != 0;
        ImGui::BeginDisabled(busy);
        ImGui::SetNextItemWidth(60.0f);
        const bool enter = ImGui::InputTextWithHint("##country", "CA", st.country, sizeof(st.country),
        ...
        if (ImGui::Button("Save##country") || (enter && changed && st.country[0])) app.SaveCountry(false);
        ImGui::EndDisabled();
        if (!id.country.empty()) {
            ImGui::SameLine();
            ImGui::PushStyleColor(ImGuiCol_Button, kPanelHi);
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered, kRed);
            if (ImGui::Button("Remove##country")) app.SaveCountry(true);
            ImGui::PopStyleColor(2);
        }
        ImGui::EndDisabled();
        if (busy) ImGui::TextDisabled("Saving...");
    ...
    // -- password ----------------------------------------------------------
    xlive::theme::Section("Password");
    ImGui::PushStyleColor(ImGuiCol_Text, kMuted);
    ImGui::TextWrapped("Changing it signs every other device out of this account. This one stays "
                       "signed in.");
    ImGui::PopStyleColor();
    {
        ...
        ImGui::TextUnformatted("New password again");
        ImGui::SetNextItemWidth(box);
        ...
        if (ImGui::Button("Change password") || (enter && ready)) app.ChangePassword();
        ImGui::EndDisabled();
        ImGui::EndDisabled();
        if (busy) ImGui::TextDisabled("Changing...");

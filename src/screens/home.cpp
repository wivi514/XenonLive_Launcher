...
std::string Megabytes(uint64_t bytes) {
    char buf[32];
    std::snprintf(buf, sizeof(buf), T("%.1f MB"), double(bytes) / (1024.0 * 1024.0));
    return buf;
}
...
void DrawGame(App& app, const CatalogGame& game) {
    ...
    if (working) {
        ...
        if (progress.phase == InstallPhase::Downloading && progress.total > 0) {
            ...
            ImGui::ProgressBar(float(double(progress.done) / double(progress.total)),
                               ImVec2(-100.0f, 0.0f), T("unpacking"));
        } else {
            ImGui::ProgressBar(-1.0f * float(ImGui::GetTime()), ImVec2(-100.0f, 0.0f), phase);
        }
        ImGui::SameLine();
        if (xlive::theme::SmallSecondaryButton(T("Cancel"))) app.installer.Cancel();
    } else if (!entry) {
        ImGui::TextDisabled(T("not installed"));
        if (latest != app.latest_tags.end()) {
            ImGui::SameLine();
            ImGui::TextDisabled(T("- latest is %s"), latest->second.c_str());
        }
    } else {
        ImGui::TextColored(kGreen, T("installed %s"), entry->version.c_str());
        if (update_available) {
            ImGui::SameLine();
            ImGui::TextColored(kAmber, T("- %s is available"), latest->second.c_str());
        } else if (latest != app.latest_tags.end()) {
            ImGui::SameLine();
            ImGui::TextDisabled(T("- up to date"));
        }
    }
    ...
    if (entry) {
        ...
        if (state == "ready") {
            ImGui::TextDisabled(T("your game data is installed"));
        } else if (state == T("package found")) {
            ImGui::TextDisabled(T("your package is in place; the first Play unpacks it"));
        } else {
            ImGui::TextColored(kAmber, T("put your XBLA package (title %s) in:"),
                               TitleIdHex(game.title_id).c_str());
            ImGui::PushTextWrapPos(0.0f);
            ImGui::TextDisabled("%s", package_dir.c_str());
            ImGui::PopTextWrapPos();
            ImGui::TextDisabled(T("or drop it onto the game's own window after pressing Play"));
        }
    }
    ...
    if (!entry) {
        if (ImGui::Button(T("Install"), ImVec2(90.0f, 0.0f))) app.InstallGame(game);
    } else if (running_this) {
        ImGui::TextColored(kGreen, T("running (pid %lld)"), (long long)app.game.pid());
    } else {
        ImGui::BeginDisabled(app.game.running());
        if (ImGui::Button(T("Play"), ImVec2(90.0f, 0.0f))) {
            std::string launch_error;
            if (!app.Launch(index, launch_error)) {
                app.toasts.Push(T("Could not launch: ") + launch_error, 8.0);
            }
        }
        ImGui::EndDisabled();
        ImGui::SameLine();
        if (update_available) {
            if (ImGui::Button((T("Update to ") + latest->second).c_str())) app.InstallGame(game);
        } else {
            if (xlive::theme::SmallSecondaryButton(T("Reinstall"))) app.InstallGame(game);
        }
        ImGui::SameLine();
        if (xlive::theme::SmallSecondaryButton(T("Folder"))) OpenFolder(entry->cwd);
    }
    const auto page = app.release_pages.find(game.key);
    if (page != app.release_pages.end() && !page->second.empty()) {
        ImGui::SameLine();
        if (xlive::theme::SmallSecondaryButton(T("Release notes"))) SDL_OpenURL(page->second.c_str());
    }
    ImGui::EndDisabled();
    ImGui::EndGroup();
...
void DrawHome(App& app) {
    ...
    if (!app.launcher_update.empty() || app.launcher_updating()) {
        ...
        ImGui::PopStyleColor();
        ImGui::PushFont(app.fonts.heading);
        ImGui::TextColored(kAmber, T("XenonLive Launcher %s is available"), app.launcher_update.c_str());
        ImGui::PopFont();
        ImGui::SameLine();
        ImGui::TextDisabled(T("you have %s"), LauncherVersion());
        if (working) {
            const char* phase = PhaseName(progress.phase);
            if (progress.phase == InstallPhase::Downloading && progress.total > 0) {
                char overlay[64];
            ...
            ImGui::TextDisabled(T("The launcher restarts by itself when this is done."));
        } else {
            ImGui::PushTextWrapPos(0.0f);
            ImGui::TextDisabled(T("Downloaded beside this one and checked against the release's SHA256SUMS, then the launcher restarts as the new version."));
            ImGui::PopTextWrapPos();
            if (ImGui::Button(T("Update and restart"))) app.UpdateLauncher();
    ...
    ImGui::PopFont();
    ImGui::SameLine();
    ImGui::TextDisabled(T("  %u G"), id.gamerscore);
    // The library's status line is English ("signed in as X", "offline:
    // retrying in 4s"); the common case gets the launcher's words.
    if (app.client->online()) ImGui::TextDisabled("%s%s", T("Signed in as "), id.gamertag.c_str());
    else ImGui::TextDisabled("%s", app.client->status().c_str());
    ImGui::TextDisabled("%s", app.client->gateway_connected() ? T("live updates on")
                                                               : T("live updates off"));
    ImGui::SameLine(ImGui::GetContentRegionAvail().x - 262.0f);
    if (xlive::theme::SmallSecondaryButton(T("Account"))) app.OpenAccount();
    ImGui::SameLine();
    if (xlive::theme::SmallSecondaryButton(T("Switch account"))) ImGui::OpenPopup("switch");
    ImGui::SameLine();
    if (xlive::theme::SmallSecondaryButton(T("Sign out"))) app.SignOut();
    if (ImGui::BeginPopup("switch")) {
        for (const SavedAccount& account : app.accounts.list()) {
            if (account.xuid == id.xuid) continue;
            const std::string label =
                account.gamertag + (account.has_tokens() ? "" : T("  (needs password)"));
        ...
        if (ImGui::Selectable(T("Add another account..."))) {
    ...
    ImGui::PushTextWrapPos(0.0f);
    ImGui::TextDisabled(T("Releases come from GitHub as %s builds (this launcher's own kind), checked against the release's SHA256SUMS. You supply your own copy of each game."),
    ...
    if (app.release_check_running()) {
        ImGui::TextDisabled(T("checking for new releases..."));
    } else if (!app.release_check_error.empty()) {
        ImGui::TextColored(kAmber, T("could not check for releases: %s"), app.release_check_error.c_str());
        ImGui::SameLine();
        ImGui::TextDisabled(T("- tried again every 5 minutes"));
    } else if (app.last_release_check >= 0.0) {
        const int ago = int((ImGui::GetTime() - app.last_release_check) / 60.0);
        if (ago < 1) ImGui::TextDisabled(T("releases checked just now - again in 5 minutes"));
        else ImGui::TextDisabled(T("releases checked %d min ago - again every 5 minutes"), ago);
    }
    for (const CatalogGame& game : Catalog()) DrawGame(app, game);
    ImGui::PushTextWrapPos(0.0f);
    ImGui::TextDisabled(T("installed under %s"),
    ...
    ImGui::Spacing();
    xlive::theme::Section(T("Builds from launcher.json"));

...
void DrawSignIn(App& app) {
    ...
    if (app.adding_account) {
        ...
        if (xlive::theme::SmallSecondaryButton(T("Back"))) app.adding_account = false;
    }
    ImGui::Separator();
    ImGui::Spacing();
    ...
    if (any) {
        ImGui::TextUnformatted(T("Saved accounts"));
        uint64_t forget = 0;
        for (const SavedAccount& account : app.accounts.list()) {
            if (account.xuid == current) continue;
    ...
    }
    {
        ...
        case App::SignInState::Mode::SignIn:
            heading = app.adding_account ? T("Add another account") : (any ? T("Or sign in") : nullptr);
            break;
        case App::SignInState::Mode::Register: heading = T("Create an account"); break;
        case App::SignInState::Mode::Forgot: heading = T("Forgot your password"); break;
        }
        if (heading) {
    ...
    case Mode::SignIn: {
        if (!st.sent_to.empty()) {
            ImGui::TextColored(xlive::theme::kLime, T("A new password was sent to %s."), st.sent_to.c_str());
            ImGui::PushStyleColor(ImGuiCol_Text, xlive::theme::kMuted);
            ImGui::TextWrapped(T("Sign in with it here (check spam if it is not there). It works for 24 hours; your old password still works until you use the new one. Once in, change it under Account."));
            ImGui::PopStyleColor();
            ImGui::Spacing();
        }
        ImGui::TextUnformatted(T("Gamertag"));
        ImGui::SetNextItemWidth(-1.0f);
        ImGui::InputText("##gamertag", st.gamertag, sizeof(st.gamertag));
        ImGui::TextUnformatted(T("Password"));
        ImGui::SetNextItemWidth(-1.0f);
        const bool enter = ImGui::InputText("##password", st.password, sizeof(st.password),
                                            ImGuiInputTextFlags_Password |
                                                ImGuiInputTextFlags_EnterReturnsTrue);
        ImGui::Spacing();
        if ((ImGui::Button(T("Sign in"), ImVec2(-1.0f, 0.0f)) || enter) && app.client) {
            use_server();
            start(Pending::SignIn, app.client->SignIn(st.gamertag, st.password));
        }
        ImGui::Spacing();
        if (ImGui::TextLink(T("Create an account"))) switch_to(Mode::Register);
        ImGui::SameLine(0.0f, 24.0f);
        if (ImGui::TextLink(T("Forgot your password?"))) switch_to(Mode::Forgot);
        break;
    }
    case Mode::Register: {
        ImGui::TextUnformatted(T("Gamertag"));
        ...
        ImGui::InputText("##password", st.password, sizeof(st.password),
                         ImGuiInputTextFlags_Password);
        ImGui::TextUnformatted(T("Email"));
        ImGui::SameLine();
        ImGui::TextColored(xlive::theme::kMuted, T("optional"));
        ImGui::SetNextItemWidth(-1.0f);
        ...
        ImGui::PushStyleColor(ImGuiCol_Text, xlive::theme::kMuted);
        ImGui::TextWrapped(T("Only used to get back in if you forget your password. Nothing else is ever sent to it: no news, no updates, nothing. It is never shown to other players."));
        ImGui::TextWrapped(T("Without one, a forgotten password means the account is lost."));
        ImGui::PopStyleColor();
        ImGui::Spacing();
        if ((ImGui::Button(T("Create account"), ImVec2(-1.0f, 0.0f)) || enter) && app.client) {
            use_server();
            start(Pending::Register, app.client->Register(st.gamertag, st.password, st.email));
        }
        ImGui::Spacing();
        if (ImGui::TextLink(T("Already have an account? Sign in"))) switch_to(Mode::SignIn);
        break;
    }
    case Mode::Forgot: {
        ImGui::TextUnformatted(T("Gamertag"));
        ...
        const bool enter = ImGui::InputText("##gamertag", st.gamertag, sizeof(st.gamertag),
                                            ImGuiInputTextFlags_EnterReturnsTrue);
        ImGui::PushStyleColor(ImGuiCol_Text, xlive::theme::kMuted);
        ImGui::TextWrapped(T("A new password goes to the email you gave when you created the account. Your current password keeps working until you sign in with the new one; change it afterwards under Account. An account created without an email cannot be recovered."));
        ImGui::PopStyleColor();
        ImGui::Spacing();
        if ((ImGui::Button(T("Email me a new password"), ImVec2(-1.0f, 0.0f)) || enter) && app.client) {
            use_server();
            start(Pending::Forgot, app.client->ForgotPassword(st.gamertag));
        }
        ImGui::Spacing();
        if (ImGui::TextLink(T("Back to sign in"))) switch_to(Mode::SignIn);
        break;
    }
    }
    ImGui::EndDisabled();
    ...
    ImGui::Spacing();
    if (ImGui::CollapsingHeader(T("Advanced: server"))) {
        ImGui::BeginDisabled(busy);
        ImGui::SetNextItemWidth(-1.0f);
        ...
        if (app.client && std::string(app.signin.server) != app.config.server) {
            if (xlive::theme::SmallSecondaryButton(T("Use this server"))) app.RestartClient();
            ImGui::SameLine();
        }
        if (std::string(app.signin.server) != kDefaultServer) {
            if (xlive::theme::SmallSecondaryButton(T("Back to the default"))) {
        ...
        if (app.config.allow_insecure) {
            ImGui::TextColored(xlive::theme::kAmber,
                               T("allow_insecure is on in launcher.json: plain http, no certificate check"));
        }
        ImGui::EndDisabled();
    }
    ...
    ImGui::SameLine();
    DrawLanguagePicker(app, 220.0f * app.ui_scale);

    ImGui::Spacing();
    if (busy) {
        const char* doing = T("Signing in...");
        switch (st.pending) {
        case Pending::SignIn: break;
        case Pending::Register: doing = T("Creating the account..."); break;
        case Pending::Forgot: doing = T("Sending the mail..."); break;
        }
        ...
        if (e == "bad_gamertag") {
            help = T("A gamertag starts with a letter and is at most 15 letters, digits and single spaces.");
        } else if (e == "bad_password") {
            help = T("A password is at least 8 characters.");
        } else if (e == "bad_email") {
            help = T("That does not look like an email address. Leave it empty if you would rather not give one.");
        } else if (e == "taken") {
            help = T("That gamertag (or email) already has an account.");
        } else if (e == "no_account") {
            help = T("No account has that gamertag.");
        } else if (e == "no_email") {
            help = T("This account was created without an email, so there is nowhere to send a new password. It cannot be recovered.");
        } else if (e == "mail_unavailable") {
            help = T("This server cannot send email, so it cannot recover accounts.");
        } else if (e == "mail_failed") {
            help = T("The server could not send the mail just now. Try again in a minute.");
        } else if (e == "too_soon") {
            help = T("A new password was mailed a moment ago. Check the inbox (and spam); another can be sent in 15 minutes.");
        } else if (e == "rate_limited") {
            help = T("Too many tries from this address. Wait a while.");
        } else if (e == "no_server") {
            help = T("No server is configured: open Advanced and press Back to the default.");
        }
